...
vi.mock('./activities/reviewCode.js', () => ({
  reviewCode: vi.fn(),
}));
...
describe('startWorker', () => {
  ...
  it('passes reviewCode activities to Worker.create', async () => {
    await startWorker({});

    expect(mockCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        activities: expect.objectContaining({ reviewCode: expect.any(Function) }),
      })
    );
  });
